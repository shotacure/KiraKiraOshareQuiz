    ...
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ fontFamily: "'Zen Maru Gothic', sans-serif", background: 'linear-gradient(135deg,#fff0f5,#ffe4ec)' }}>
        <div className="w-full max-w-md mx-4 bg-white/80 backdrop-blur-sm border-2 border-pink-200 rounded-3xl p-8 shadow-xl">
          <h1 className="font-black text-2xl text-pink-500 mb-6 text-center">📺 表示用画面</h1>
  ...
  return (
    <div className="w-screen h-screen overflow-hidden relative" style={{
      ...
      <div className="relative z-10 w-full h-full">
        ...
        {state.status === 'judging' && <JudgingView />}
...
function Sparkles() {
  const sparkles = Array.from({ length: 20 }, (_, i) => ({
    ...
    delay: `${Math.random() * 5}s`, duration: `${3 + Math.random() * 4}s`,
  }));
  return (
    <div className="fixed inset-0 pointer-events-none z-0">
      {sparkles.map(s => (
        <div key={s.id} className="absolute animate-pulse" style={{
          left: s.left, top: s.top, animationDelay: s.delay, animationDuration: s.duration,
          fontSize: '1.5vw', opacity: 0.25, color: '#ff6b9d',
        }}>✦</div>
      ))}
    </div>
  );
...
function AcceptingView({ players, status }) {
  ...
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=600x600&data=${encodeURIComponent(playerUrl)}&color=ff6b9d&bgcolor=fff0f5`;
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center">
      ...
      {status === 'init' ? (
        <p className="font-bold text-pink-400" style={{ fontSize: '2vw' }}>まもなく開始します...</p>
      ) : (
        <>
          <div className="bg-white/70 backdrop-blur-sm border-2 border-pink-200 shadow-xl shadow-pink-100"
            style={{ borderRadius: '2vw', padding: '2vw', marginBottom: '2vh' }}>
            <p className="text-center text-pink-500 font-bold" style={{ fontSize: '1.8vw', marginBottom: '1.5vh' }}>📱 スマホでアクセスしてね！</p>
            <div className="flex items-center" style={{ gap: '3vw' }}>
              <img src={qrUrl} alt="QR" style={{ width: '22vw', height: '22vw', borderRadius: '1.5vw' }} crossOrigin="anonymous" />
              <div className="text-center">
                <p className="text-pink-400" style={{ fontSize: '1.2vw', marginBottom: '0.5vh' }}>URL</p>
                <p className="font-black text-pink-600 break-all" style={{ fontSize: '2vw', maxWidth: '20vw', wordBreak: 'break-all' }}>
                  {playerUrl}
                </p>
              </div>
            </div>
          ...
          <div className="bg-white/50 backdrop-blur-sm border border-pink-200"
            ...
            <span className="font-bold text-pink-500" style={{ fontSize: '2vw' }}>
              参加者: <span className="text-pink-600 font-black" style={{ fontSize: '2.5vw' }}>{players.length}</span> 名 🌸
            </span>
          </div>
        </>
      )}
      ...
      <div className="flex flex-col items-center justify-center" style={{ width: '68%', padding: '2vw' }}>
        ...
        <div style={{ width: '50vw', marginTop: '1.5vh' }}>
          <div className="flex items-center justify-between" style={{ marginBottom: '0.5vh' }}>
            <span className="font-bold text-pink-500" style={{ fontSize: '1.3vw' }}>回答してね！💖</span>
      ...
      <div className="flex flex-col border-l border-pink-200 bg-white/30" style={{ width: '32%', padding: '1.5vw' }}>
        ...
        <div className="flex-1 overflow-hidden">
          {liveCorrectPlayers.length === 0 ? (
            <p className="text-pink-300 text-center" style={{ fontSize: '1.2vw', marginTop: '3vh' }}>まだいないよ...</p>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6vw' }}>
              {liveCorrectPlayers.slice(0, 12).map((p, i) => (
                <div key={p.playerId} className="flex items-center bg-white/60 border border-pink-100 animate-slide-up"
                  ...
                  <span className="text-pink-300 font-mono shrink-0" style={{ fontSize: '0.9vw' }}>
...
function JudgingView() {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center">
      ...
      <p className="font-black text-pink-500" style={{ fontSize: '3.5vw' }}>採点中...</p>
      <p className="text-pink-300 font-bold" style={{ fontSize: '1.8vw', marginTop: '1vh' }}>ちょっと待っててね！</p>
    </div>
  );
}
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center overflow-auto" style={{ padding: '2vw' }}>
      ...
      <div className="text-center" style={{ marginBottom: '2.5vh' }}>
        <p className="font-bold text-pink-400" style={{ fontSize: '1.8vw', marginBottom: '1vh' }}>✨ 正解 ✨</p>
      ...
      <div className="flex" style={{ gap: '3vw', marginBottom: '2vh' }}>
        <div className="text-center bg-white/60 border border-green-200" style={{ borderRadius: '1.5vw', padding: '0.8vw 2vw' }}>
          ...
          <p className="font-bold text-green-400" style={{ fontSize: '1vw' }}>正解 🎉</p>
        </div>
        <div className="text-center bg-white/60 border border-red-200" style={{ borderRadius: '1.5vw', padding: '0.8vw 2vw' }}>
          ...
          <p className="font-bold text-red-300" style={{ fontSize: '1vw' }}>不正解 😢</p>
        </div>
      </div>

      {correctPlayers.length > 0 && (
        <div style={{ width: '55vw' }}>
          <p className="font-bold text-pink-500 text-center" style={{ fontSize: '1.5vw', marginBottom: '1vh' }}>正解者（はやい順）🏃‍♀️</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6vw' }}>
            {correctPlayers.slice(0, 10).map((p, i) => (
              <div key={p.playerId} className="flex items-center bg-white/70 border-2 border-pink-100 animate-slide-up shadow-sm"
                ...
                <span className="font-mono text-pink-400 shrink-0" style={{ fontSize: '1.1vw' }}>{formatElapsedMs(p.elapsedMs)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {correctPlayers.length === 0 && (
        <p className="font-bold text-pink-300" style={{ fontSize: '2vw' }}>正解者なし... 😭</p>
      )}
    </div>
  );
}
    ...
    <div className="w-full h-full flex flex-col items-center overflow-auto" style={{ padding: '2vw' }}>
      ...
      <div style={{ width: '65vw', display: 'flex', flexDirection: 'column', gap: '0.6vw' }}>
        {sorted.slice(0, 15).map((r, i) => {
          ...
          const barWidth = Math.max(5, (r.totalScore / barMax) * 100);
          return (
            <div key={r.playerId || i} className={`flex items-center animate-slide-up ${
              ...
              <div className="flex-1 bg-pink-50 border border-pink-100 overflow-hidden"
                ...
                <div className="h-full bg-gradient-to-r from-pink-400 to-rose-400 animate-fill"
                  style={{ width: `${barWidth}%`, borderRadius: '0.6vw' }} />
              </div>
